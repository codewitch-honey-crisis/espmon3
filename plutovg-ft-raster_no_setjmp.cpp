...
#define PVG_FT_MAX_GRAY_SPANS  1024
  ...
  typedef struct  TWorker_
  {
    ...
    PVG_FT_Span     gray_spans[PVG_FT_MAX_GRAY_SPANS+1];
    int         num_gray_spans;
    int         skip_spans;
  ...
  gray_compute_cbox( RAS_ARG )
  {
    ...
    vector_t*   vec     = outline->points;
    vector_t*   limit   = vec + outline->n_points;
  ...
  /*                                                                       */
  static bool
  gray_find_cell( RAS_ARG, PCell* out_pcell )
  {
    PCell  *pcell, cell;
    TPos    x = ras.ex;
  ...
  gray_record_cell( RAS_ARG )
  {
    ...
    {
      PCell  cell;
      if(!gray_find_cell( RAS_VAR ,&cell)) {
        return false;
      }
      if(cell==NULL) {
        return true;
                            ...
                            TCoord  ey )
  {
    ...
    bool res = gray_set_cell( RAS_VAR_ ex, ey );
    return res;
  }
...
// The new render-line implementation is not yet used
                             ...
                             TPos  to_y )
  {
    ...
    x = ras.x + delta;
    if(!gray_render_scanline( RAS_VAR_ ey1, ras.x, fy1, x, (TCoord)first )) {
        return false;
    }
    ...
    if ( ey1 != ey2 )
    {
      ...
      do
      {
        ...
        x2 = x + delta;
        if(!gray_render_scanline( RAS_VAR_ ey1,
                                       x, ONE_PIXEL - first,
                                       x2, first )) {
            return false;
        }
  ...
  static void
  gray_split_conic( vector_t*  base )
  {
    TPos  a, b;
  ...
  static bool
  gray_render_conic( RAS_ARG_ const vector_t*  control,
                              const vector_t*  to )
  {
    vector_t   bez_stack[16 * 2 + 1];  /* enough to accommodate bisections */
    vector_t*  arc = bez_stack;
    TPos        dx, dy;
    int         draw, split;
  ...
  static void
  gray_split_cubic( vector_t*  base )
  {
    TPos  a, b, c, d;
  ...
  static bool
  gray_render_cubic( RAS_ARG_ const vector_t*  control1,
                              const vector_t*  control2,
                              const vector_t*  to )
  {
    vector_t   bez_stack[16 * 3 + 1];  /* enough to accommodate bisections */
    vector_t*  arc = bez_stack;
    vector_t*  limit = bez_stack + 45;
    TPos        dx, dy, dx_, dy_;
  ...
  static bool
  gray_move_to( const vector_t*  to,
                PWorker           worker, int* out)
  {
    TPos  x, y;
    ...
    if(!gray_start_cell( worker, TRUNC( x ), TRUNC( y ) )) {
        return false;
    }
  ...
  /*                                                                       */
  static
  bool  PVG_FT_Outline_Decompose( const PVG_FT_Outline*        outline,
                                void*                       user , int* out_error)
  {
#undef SCALED
#define SCALED( x )  (x)
    ...
    if ( !outline ) {
      *out_error =ErrRaster_Invalid_Outline;
      return true;
    }
    ...
    for ( n = 0; n < outline->n_contours; n++ )
    {
      ...
      if(!gray_move_to( &v_start,(PWorker) user ,out_error)) {
        return false;
      }
      if ( *out_error )
        goto Exit;
        ...
        case PVG_FT_CURVE_TAG_ON:  /* emit a single line_to */
          {
            ...
            if(!gray_render_line((PWorker)user, UPSCALE(vec.x), UPSCALE(vec.y))) {
                return false;
            }
            continue;
          }
        ...
        case PVG_FT_CURVE_TAG_CONIC:  /* consume conic arcs */
          ...
          Do_Conic:
            if ( point < limit )
            {
              ...
              {
                if(!gray_render_conic((PWorker) user, &v_control, &vec)) {
                    return false;
                }
                continue;
              }
              ...
              if(!gray_render_conic((PWorker)user, &v_control, &v_middle)) {
                return false;
              }
            ...
            if(!gray_render_conic((PWorker)user, &v_control, &v_start)) {
                return false;
            }
            goto Close;
          }
        ...
        default:  /* PVG_FT_CURVE_TAG_CUBIC */
          {
            ...
            if ( point <= limit )
            {
              ...
              if(!gray_render_cubic((PWorker)user, &vec1, &vec2, &vec)) {
                return false;
              }
              continue;
            }

            if(!gray_render_cubic((PWorker)user, &vec1, &vec2, &v_start)) {
                return false;
            }
            goto Close;
          }
      ...
      if(!gray_render_line((PWorker)user, UPSCALE(v_start.x), UPSCALE(v_start.y))) {
        return false;
      }
  ...
  static bool
  gray_convert_glyph_inner( RAS_ARG, int* out_error )
  {
    // int  error = 0;

    if ( PVG_FT_Outline_Decompose( &ras.outline, &ras ,out_error) )
    {
      if ( !ras.invalid )
        if(!gray_record_cell( RAS_VAR )) {
            return false;
    ...
    {
      *out_error = ErrRaster_Memory_Overflow;
    }
    return true;
  }
  ...
  static bool
  gray_convert_glyph( RAS_ARG ,int* out_error)
  {
    TBand            bands[40];
    TBand* volatile  band;
    ...
    for ( n = 0; n < num_bands; n++, min = max )
    {
      ...
      while ( band >= bands )
      {
        ...
        if(!gray_convert_glyph_inner( RAS_VAR ,&error)) {
            return false;
        }
        if ( !error )
        {
      ...
      ReduceBands:
        ...
        if ( middle == bottom )
        {
          *out_error= ErrRaster_OutOfMemory;
          return true;
        }
  ...
  gray_raster_render( RAS_ARG_ void* buffer, long buffer_size,
                      const PVG_FT_Raster_Params*  params,int* out_error )
  {
    ...
    if ( !( params->flags & PVG_FT_RASTER_FLAG_AA ) ) {
      *out_error= ErrRaster_Invalid_Mode;
      return true;
    }
    ...
    return gray_convert_glyph( RAS_VAR ,out_error);
  }
  ...
  PVG_FT_Raster_Render(const PVG_FT_Raster_Params *params)
  {
      ...
      int error=0;
      worker.gray_spans[PVG_FT_MAX_GRAY_SPANS].coverage = 128;
      worker.gray_spans[PVG_FT_MAX_GRAY_SPANS].len = 128;
      worker.gray_spans[PVG_FT_MAX_GRAY_SPANS].x = 128;
      worker.gray_spans[PVG_FT_MAX_GRAY_SPANS].y = 128;
      gray_raster_render(&worker, memory, length, params,&error);
